...
// --- Assets (CDN) ---
const ASSET_BASE = 'https://imgcdn.cna.com.tw/missions/M2';
const ASSET_V = '1';
function assetUrl(path) {
    const p = String(path || '').replace(/^\.?\//, '');
    return `${ASSET_BASE}/${p}?v=${encodeURIComponent(ASSET_V)}`;
}
...
function getLegislatorPhotoSrc(name, party) {
    ...
    if (isCaucusName(name)) {
        ...
        if (s.includes('民進')) return assetUrl('img/dpp.svg');
        if (s.includes('國民')) return assetUrl('img/kmt..png');
        if (s.includes('民眾')) return assetUrl('img/tpp.svg');
        return makeCaucusAvatarDataUri(name, party || name);
    }
    return assetUrl(`photos/${encodeURIComponent(name)}.jpg`);
}
...
function openDetailC(jsonStr) {
    ...
    // Keep the js hook class; do not overwrite it (otherwise next open breaks)
    partyEl.classList.remove('tag-dpp', 'tag-kmt', 'tag-tpp', 'tag-other');
    partyEl.classList.add('leg-party-tag', 'js-mc-party', partyClass);
    ...
    imgEl.alt = `${(data['委員姓名'] || '').toString().trim() || '立委'} 照片`;
