...
function getLegislatorPhotoSrc(name, party) {
    ...
    // Use JPG portraits for better compatibility with current deploy pipeline.
    ...
    data.forEach(row => {
        ...
        const imgHtml = photoPath
            ? `<img src="${photoPath}" alt="${name} 照片"
                             ...
                             onerror="
                                if (!this.dataset.fallbackTried && this.src.endsWith('.webp')) {
                                    this.dataset.fallbackTried = '1';
                                    this.src = this.src.replace(/\\.webp($|\\?)/, '.jpg$1');
                                    return;
                                }
